...
#ifdef USE_CUDA
    // --- CUDA path: compute all sigmas at once, then print thresholds ---
    {
        double* sigmas = new double[arguments.totalChannelNumber]();
        runCudaMedianThreshold(input, oneChannel_nbRecords,
                               arguments.totalChannelNumber, sigmas, verbose);

        // Re-parse groups to print output in expected format
        char* groups2    = strdupa(arguments.channelList);
        char* grp2       = strsep(&groups2, GROUP_SEPARATOR);
        int   grpIdx     = 0;
        int   chListSize = 0;
        while (grp2 != NULL) {
            int   localIds[MAX_CHANNO];
            int   localN = 0;
            char* ch2    = strsep(&grp2, CHANNEL_SEPARATOR);
            while (ch2 != NULL) {
                localIds[localN++] = atoi(ch2);
                ch2 = strsep(&grp2, CHANNEL_SEPARATOR);
                if (ch2 == NULL) break;
            }
            chListSize += localN;
            if (grpIdx > 0) cout << ":";
            for (int c = 0; c < localN; ++c) {
                int chId = localIds[c];
                long double thr = (long double)(4.0 * sigmas[chId]);
                if (c > 0) cout << ",";
                cout << thr;
            }
            grp2 = strsep(&groups2, GROUP_SEPARATOR);
            ++grpIdx;
            if (grp2 == NULL) break;
        }
        cout << endl;
        delete[] sigmas;
        delete[] input; delete[] groupChannelId; delete[] thresholds;
...
long double* groupThresholds(const short* allRecords, int allChannelsNb,
	...
	if ( verbose ) cout << "Computing medians..." << endl;
	for(int i = 0; i < groupChannelNb; i++) {
		// O(N) median via nth_element instead of O(N log N) full sort
		sigma_n = medianOf(records[i], oneChannel_nbRecords) / SIGMA_DIVISOR;
