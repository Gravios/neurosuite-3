...
void PipelineCanvas::loadPreset(const QVector<QString>& types)
{
    ...
    for (int i = 0; i + 1 < m_nodes->size(); ++i)
        m_edges->append({ (*m_nodes)[i].id, (*m_nodes)[i + 1].id });
...
void PipelineCanvas::deleteSelected()
{
    ...
    if (!m_selNodeId.isEmpty()) {
        ...
        m_nodes->removeIf([&](const PipelineNode& n) { return n.id == m_selNodeId; });
        m_selNodeId.clear();
    ...
    if (!m_selEdgeKey.isEmpty()) {
        const QStringList parts = m_selEdgeKey.split(QChar(0x2192)); // →
        if (parts.size() == 2) {
            m_edges->removeIf([&](const PipelineEdge& e) {
...
PipelineNode* PipelineCanvas::nodeAt(const QPointF& world)
{
    ...
    for (int i = m_nodes->size() - 1; i >= 0; --i) {
        if (nodeRect((*m_nodes)[i]).contains(world))
            return &(*m_nodes)[i];
    }
    return nullptr;
}
...
void PipelineCanvas::drawGrid(QPainter& p)
{
    ...
    const float ox   = float(std::fmod(m_pan.x(), step));
    const float oy   = float(std::fmod(m_pan.y(), step));
...
void PipelineCanvas::drawNode(QPainter& p, const PipelineNode& n)
{
    ...
    const bool    isSel     = (n.id == m_selNodeId);
...
{
    if (!m_connecting || m_connectFrom.isEmpty() || !m_nodes) return;
    for (const PipelineNode& n : *m_nodes) {
        if (n.id != m_connectFrom) continue;
        const QPointF p1 = w2s(outPort(n));
        p.setPen(QPen(QColor(0x4a, 0x9e, 0xff, 180), 2, Qt::DashLine));
        p.setBrush(Qt::NoBrush);
        p.drawPath(edgePath(p1, m_connectCursor));
        break;
    }
}
...
{
    if (m_panActive) {
        m_pan = ev->position() - m_panStart;
        update();
        return;
    }
    if (!m_dragId.isEmpty() && m_nodes) {
        const QPointF world = s2w(ev->position());
        for (PipelineNode& n : *m_nodes) {
            if (n.id == m_dragId) {
                n.pos = world - m_dragOffset;
...
void PipelineCanvas::mouseReleaseEvent(QMouseEvent* ev)
{
    ...
    if (m_connecting && ev->button() == Qt::LeftButton && m_nodes && m_edges) {
        const QPointF world = s2w(ev->position());
        for (PipelineNode& n : *m_nodes) {
            if (n.id != m_connectFrom && hitIn(n, world)) {
                ...
                bool dup = false;
                for (const PipelineEdge& e : *m_edges)
                    if (e.from == m_connectFrom && e.to == n.id) { dup = true; break; }
                if (!dup)
                    m_edges->append({ m_connectFrom, n.id });
                emit graphModified();
                break;
            }
        }
...
PipelineDesignerPage::PipelineDesignerPage(QWidget* parent) : QWidget(parent)
{
    ...
    m_presetCombo->setFixedWidth(220);
    ...
    m_presetCombo->addItem(tr("— select preset —"), QVariant());
    m_presetCombo->addItem(tr("Pipeline A  (raw → align → PCA → KK)"),
        QVariant(QStringList{"ndm_hipass","ndm_extractspikes","ndm_alignspikes","ndm_pca","ndm_klustakwik"}));
    m_presetCombo->addItem(tr("Pipeline C  (raw → align → StDeriv PCA → KK)"),
        QVariant(QStringList{"ndm_hipass","ndm_extractspikes","ndm_alignspikes","ndm_pca_stderiv","ndm_klustakwik"}));
    m_presetCombo->addItem(tr("Pipeline D  (StDeriv detect → align → PCA → KK)"),
        QVariant(QStringList{"ndm_hipass","ndm_extractspikes_stderiv","ndm_alignspikes","ndm_pca","ndm_klustakwik"}));
    m_presetCombo->addItem(tr("Full pipeline (D + reextract + drift + LFP)"),
    ...
    connect(m_applyBtn, &QPushButton::clicked, this, &PipelineDesignerPage::onApply);
    tbox->addWidget(m_applyBtn);
    ...
    palVbox->addWidget(m_palette);
    splitter->addWidget(palWrapper);

    // ── Canvas ────────────────────────────────────────────────────────────
    m_canvas = new PipelineCanvas;
    m_canvas->bind(&m_nodes, &m_edges);
    connect(m_canvas, &PipelineCanvas::nodeSelected,   this, &PipelineDesignerPage::onNodeSelected);
    connect(m_canvas, &PipelineCanvas::selectionCleared, this, &PipelineDesignerPage::onSelectionCleared);
    connect(m_canvas, &PipelineCanvas::graphModified,  this, &PipelineDesignerPage::onGraphModified);
    splitter->addWidget(m_canvas);

    // ── Inspector ─────────────────────────────────────────────────────────
    buildInspector();
    splitter->addWidget(m_inspScroll);
...
void PipelineDesignerPage::buildPalette()
{
    ...
    m_palette->setIconSize(QSize(8, 8));
...
void PipelineDesignerPage::buildInspector()
{
    ...
    m_inspScroll->setFixedWidth(265);
    m_inspScroll->setFrameShape(QFrame::NoFrame);
    m_inspScroll->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    m_inspScroll->setWidgetResizable(true);
    m_inspScroll->setStyleSheet(
    ...
    auto* ivbox = new QVBoxLayout(m_inspWidget);
    ...
    m_inspTitle = new QLabel(tr("Select a node to inspect"));
    m_inspTitle->setWordWrap(true);
    m_inspTitle->setStyleSheet("color:#6b7280;font-size:11px;padding:8px 0;");
    ivbox->addWidget(m_inspTitle);

    m_inspEnabled = new QCheckBox(tr("Enabled"));
    m_inspEnabled->setVisible(false);
    m_inspEnabled->setStyleSheet(
    ...
    connect(m_inspEnabled, &QCheckBox::toggled, this, &PipelineDesignerPage::onEnabledToggled);
    ivbox->addWidget(m_inspEnabled);

    // Parameter form area
    m_inspParamArea = new QWidget;
    m_inspParamArea->setVisible(false);
    auto* paramVbox = new QVBoxLayout(m_inspParamArea);
    ...
    m_inspForm = new QFormLayout(formWidget);
    m_inspForm->setContentsMargins(0, 0, 0, 0);
    m_inspForm->setSpacing(6);
    m_inspForm->setLabelAlignment(Qt::AlignLeft);
    paramVbox->addWidget(formWidget);
    ivbox->addWidget(m_inspParamArea);
    ivbox->addStretch();
}
...
{
    if (m_inspectedId.isEmpty()) return;
    for (PipelineNode& n : m_nodes) {
        if (n.id != m_inspectedId) continue;
        for (int i = 0; i < m_paramEdits.size() && i < n.params.size(); ++i)
            n.params[i].second = m_paramEdits[i]->text();
        break;
    }
    onGraphModified();
}
...
void PipelineDesignerPage::onEnabledToggled(bool checked)
{
    if (m_inspectedId.isEmpty()) return;
    for (PipelineNode& n : m_nodes) {
        if (n.id == m_inspectedId) { n.enabled = checked; break; }
    }
    if (m_canvas) m_canvas->update();
    onGraphModified();
}
...
{
    const QVariant data = m_presetCombo->itemData(index);
    if (!data.isValid()) return;
    const QStringList types = data.toStringList();
    if (types.isEmpty()) return;
    QVector<QString> tv(types.begin(), types.end());
    if (m_canvas) m_canvas->loadPreset(tv);
    m_presetCombo->setCurrentIndex(0); // reset combo after loading
}

void PipelineDesignerPage::onPaletteDoubleClicked(QListWidgetItem* item)
{
    const QString type = item->data(Qt::UserRole).toString();
    if (type.isEmpty() || !m_canvas) return;
    ...
    if (def)
        for (const NdmParamDef& p : def->params)
            n.params.append({ p.name, p.defaultValue });
    m_nodes.append(n);
    if (m_canvas->selectedNodeId().isEmpty() && m_nodes.size() > 1) {
        // auto-connect to the previous last node
        const QString prevId = m_nodes[m_nodes.size()-2].id;
        m_edges.append({ prevId, n.id });
    }
    if (m_canvas) {
        m_canvas->selectNode(n.id);
        emit m_canvas->graphModified();
    }
}
...
void PipelineDesignerPage::populateInspector(const QString& id)
{
    ...
    for (const auto& pv : node->params) {
        ...
        m_inspForm->addRow(lbl, ed);
        m_paramEdits.append(ed);
    }
    ...
    if (node->params.isEmpty()) {
        ...
        m_inspForm->addRow(noParam);
    }

    m_inspParamArea->setVisible(true);
}

void PipelineDesignerPage::clearInspector()
{
    m_inspTitle->setText(tr("Select a node"));
    m_inspTitle->setStyleSheet("color:#6b7280;font-size:11px;padding:8px 0;");
    m_inspEnabled->setVisible(false);
    m_inspParamArea->setVisible(false);
    while (m_inspForm->rowCount() > 0) m_inspForm->removeRow(0);
...
{
    m_applyBtn->setEnabled(!m_nodes.isEmpty());
    m_applyBtn->setStyleSheet(m_modified && !m_nodes.isEmpty() ?
